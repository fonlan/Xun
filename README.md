...
- 服务状态提示：首次启动若已触发服务拉起（含 UAC 提权已通过），在服务可连接前输入框占位符会提示“服务端启动中，请稍后...”；仅在未启动/无权限/UAC 取消等情况下提示“服务端未启动”
- 搜索中动效：输入框发起查询后会显示底部轻量流动指示条，结果返回或查询取消后立即消失
...
- 查询模式切换：输入框默认“通配”模式（支持 `*` 与 `?`）且默认仅匹配文件名；`Full` 开关可切换为全路径匹配；可通过右侧 `.*` 开关切换到“正则”模式；`Aa` 开关可切换大小写敏感（默认不区分大小写）
- 打开历史置顶：客户端会记录曾打开项；后续搜索命中这些项时优先置顶显示
- 文件类型筛选：全部、可执行、压缩、文本、办公、音频、视频、图片、其他
- 结果列表右键菜单：支持“打开 / 打开（管理员权限） / 打开目录 / 复制 / 剪切 / 复制文件名 / 复制目录名 / 复制完整路径”
